...
const Login = () => {
    ...
    const onSubmitHandler = async (e) => {
      // Lỗi: prevenDefault phải là preventDefault
      e.preventDefault();
      ...
      try{
        if(state === 'Login'){
          ...
          if(data.success){
            setToken(data.token)
            setUser(data.user)
            // Lỗi: localStorage.getItem phải là localStorage.setItem
      ...
      // Lỗi: catch {error} phải là catch (error)
      }catch (error){
        toast.error(error.message)
      }
    }
    
    // Lỗi: useEffect không được đặt bên trong onSubmitHandler
    useEffect(()=>{
...
:
    <p className='mt-5 text-center'>Already have have an account? 
